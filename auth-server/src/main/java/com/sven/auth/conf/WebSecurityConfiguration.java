...
public class WebSecurityConfiguration {
    ...
    SecurityFilterChain resources(HttpSecurity httpSecurity) throws Exception {
        httpSecurity
                .securityMatchers((matchers) -> matchers.requestMatchers("/actuator/**", "/css/**", "/error", "/test/captcha"))
                                    ...
                                    .userInfoEndpoint(userInfoEndpoint -> {
                                        userInfoEndpoint
                                                .userInfoRequestConverter(
                                                        new CustomOidcUserInfoRequestConverter(authorizationService))
                                                .userInfoMapper(context -> userInfoMapper(context));
                                    });
                        }))
                .exceptionHandling(exception -> exception
                        .authenticationEntryPoint(new LoginUrlAuthenticationEntryPoint("/login")) // 认证失败异常处理
                )
